/*
  # Solomon Bharat B2B Marketplace Schema
...
CREATE POLICY "Buyers can manage own RFQs"
  ...
  FOR ALL
  TO authenticated
  USING (buyer_id = uid() OR is_admin())
  WITH CHECK (buyer_id = uid() OR is_admin());
...
CREATE POLICY "Users can read relevant RFQ assignments"
  ...
  USING (
    supplier_id = uid() OR 
    is_admin() OR 
    EXISTS (SELECT 1 FROM rfqs WHERE id = rfq_suppliers.rfq_id AND buyer_id = uid())
  );
...
CREATE POLICY "Suppliers can manage own quotations"
  ...
  FOR ALL
  TO authenticated
  USING (supplier_id = uid() OR is_admin())
  WITH CHECK (supplier_id = uid() OR is_admin());
...
CREATE POLICY "Buyers can read approved quotations for own RFQs"
  ...
  USING (
    ...
    (status = 'approved_for_buyer' AND EXISTS (
      SELECT 1 FROM rfqs WHERE id = supplier_quotations.rfq_id AND buyer_id = uid()
    ))
  );

CREATE POLICY "Suppliers can create quotations when allowed"
  ON supplier_quotations
  FOR INSERT
  TO authenticated
  WITH CHECK (
    supplier_id = uid() AND (
      ...
      EXISTS (
        ...
        WHERE r.id = supplier_quotations.rfq_id 
        AND r.open_for_bidding = TRUE 
        AND r.category = ANY(s.product_categories)
      )
    )
  );
...
CREATE POLICY "Buyers can create sample requests for own RFQs"
  ...
  WITH CHECK (
    buyer_id = uid() AND 
    EXISTS (SELECT 1 FROM rfqs WHERE id = sample_requests.rfq_id AND buyer_id = uid())
  );
...
CREATE POLICY "Users can read own sample requests"
  ...
  FOR SELECT
  TO authenticated
  USING (buyer_id = uid() OR supplier_id = uid() OR is_admin());

CREATE POLICY "Suppliers can update own sample tracking"
  ON sample_requests
  FOR UPDATE
  TO authenticated
  USING (supplier_id = uid() AND status = 'approved_by_admin')
  WITH CHECK (supplier_id = uid() AND status = 'approved_by_admin');
  ...
  WITH CHECK (
    supplier_id = uid() AND (
      ...
      EXISTS (
        SELECT 1 FROM rfqs r 
        JOIN suppliers s ON s.id = uid()
        WHERE r.id = rfq_questions.rfq_id 
        AND r.open_for_bidding = TRUE 
        AND r.category = ANY(s.product_categories)
      )
    )
...
CREATE POLICY "Users can read relevant questions"
  ...
  USING (
    supplier_id = uid() OR 
    is_admin() OR 
    EXISTS (SELECT 1 FROM rfqs WHERE id = rfq_questions.rfq_id AND buyer_id = uid())
  );
